...
-- ============================================================================
-- 10. EXERCISE DETAIL TOUR (for standalone exercise detail view)
-- ============================================================================
INSERT INTO public.content (key, content_type, platforms, title, body, order_index, active, metadata)
VALUES
...
('tour.screen.CreateRouteSheet.create-save', 'tour', ARRAY['mobile', 'both'],
 ...
 '{"en": "When you''re happy, tap Save! Your route will appear on the map for others to discover. You can set it as private if you only want to keep it for yourself.", "sv": "När du är nöjd, tryck Spara! Din rutt visas på kartan så andra kan upptäcka den. Du kan sätta den som privat om du bara vill behålla den för dig själv."}'::jsonb,
 9, true,
...
('tour.screen.RecordDrivingSheet.record-start', 'tour', ARRAY['mobile', 'both'],
 ...
 '{"en": "Tap the play button when you''re ready to start driving. The recording begins immediately - no need to rush, take your time to get comfortable!", "sv": "Tryck på play-knappen när du är redo att börja köra. Inspelningen börjar direkt - ingen brådska, ta din tid att bli bekväm!"}'::jsonb,
 2, true,
...
('tour.screen.RecordDrivingSheet.record-pause', 'tour', ARRAY['mobile', 'both'],
 ...
 '{"en": "Taking a break? Tap pause to stop recording temporarily. The app won''t track while paused - perfect for rest stops or if you need to step out.", "sv": "Tar du en paus? Tryck på pausa för att stoppa inspelningen tillfälligt. Appen spårar inte medan den är pausad - perfekt för rastplatser eller om du behöver gå ut."}'::jsonb,
 5, true,
...
('tour.screen.RecordDrivingSheet.record-stop', 'tour', ARRAY['mobile', 'both'],
 ...
 '{"en": "Done driving? Tap the red stop button. You''ll see a summary of your route with all the waypoints captured during your drive.", "sv": "Kört klart? Tryck på den röda stoppknappen. Du ser en sammanfattning av din rutt med alla vägpunkter som fångades under din körning."}'::jsonb,
 7, true,
