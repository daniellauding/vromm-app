...
export function UnlockProvider({ children }: { children: React.ReactNode }) {
  ...
  const addUnlockedPath = useCallback(async (pathId: string) => {
    ...
    try {
      const { error } = await supabase.from('user_unlocked_content').upsert({
  ...
  const loadUserPayments = useCallback(async (userId: string) => {
    ...
    try {
      ...
      if (!error && payments) {
        ...
        console.log(
          '🔓 [UnlockContext] Loaded user payments:',
          payments.length,
          payments.map((p) => ({
            id: p.id,
  ...
  const hasPathPayment = useCallback(
    (pathId: string) => {
      return userPayments.some((payment) => {
        ...
        return (
          (hasDirectField || hasMetadataField || hasFeatureKey) && payment.status === 'completed'
        );
      });
    },
    [userPayments],
  ...
  const hasExercisePayment = useCallback(
    (exerciseId: string) => {
  ...
  const value: UnlockContextType = React.useMemo(
    () => ({
      unlockedPaths,
      unlockedExercises,
      userPayments,
