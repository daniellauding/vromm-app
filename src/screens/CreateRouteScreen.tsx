...
import { RouteExerciseSelector } from '../components/shared/RouteExerciseSelector';
...
export function CreateRouteScreen({ route, isModal, hideHeader }: Props) {
  ...
  const onboardingStep = route?.params?.onboardingStep;
  ...
  const handleWaypointMode = async (latitude: number, longitude: number) => {
    try {
      ...
      // Performance info for Expo Go (but don't limit)
      if (waypointNumber > 10) {
        console.warn(`Performance note: ${waypointNumber} waypoints - consider testing on device for best performance`);
      }
  ...
  const handleCreateDraft = async () => {
    ...
    try {
      ...
      if (fromOnboarding) {
        // If from onboarding, go back to continue onboarding flow
        console.log('🎯 [CreateRoute] Returning to onboarding after draft save');
        navigation.goBack();
      } else if (navigation) {
        // @ts-ignore
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (fromOnboarding) {
        // If from onboarding, go back to continue onboarding flow
        console.log('🎯 [CreateRoute] Returning to onboarding after route creation');
        navigation.goBack();
