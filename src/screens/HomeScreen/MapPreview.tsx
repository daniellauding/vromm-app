...
export const MapPreview = React.memo(function MapPreview() {
  ...
  React.useEffect(() => {
    console.log('[MapPreview] Location context:', {
      userLocation: userLocation ? { name: userLocation.name, lat: userLocation.latitude, lng: userLocation.longitude, source: userLocation.source } : null,
      currentLocation: currentLocation ? { lat: currentLocation.coords.latitude, lng: currentLocation.coords.longitude } : null,
    });
  ...
  const effectiveLocation = React.useMemo(() => {
    ...
    if (userLocation && userLocation.latitude && userLocation.longitude) {
      console.log('[MapPreview] ✅ Using saved location:', userLocation.name, userLocation.latitude, userLocation.longitude);
      return {
    ...
    if (currentLocation?.coords) {
      console.log('[MapPreview] 📍 Using GPS location');
      return {
    ...
    console.log('[MapPreview] ⚠️ No location available, using Stockholm default');
    return {
      latitude: 59.3293,
      longitude: 18.0686,
    };
  ...
  React.useEffect(() => {
    ...
    const fetchNearbyRoutes = async () => {
      try {
        console.log('[MapPreview] Fetching routes near:', effectiveLocation);
        ...
        console.log('[MapPreview] Found routes:', data?.length || 0);
        ...
        const routeMarkers: RouteMarker[] = (data || []).slice(0, 15).map((route: any) => {
          ...
          if (!firstWp) {
            console.log('[MapPreview] Route missing waypoints:', route.id, route.name);
            return null;
          }
          ...
          if (isNaN(lat) || isNaN(lng)) {
            console.log('[MapPreview] Invalid coordinates for route:', route.id);
            return null;
          }
        ...
        console.log('[MapPreview] Valid markers:', routeMarkers.length);
  ...
  const region = {
    ...
    latitudeDelta: 0.06, // Wider view to show more pins
    longitudeDelta: 0.06,
  };
  ...
  const labelText = language === 'sv' ? 'Utforska kartan' : 'Explore map';
