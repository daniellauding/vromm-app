...
export function MapScreen({
  ...
  const { activeRoutes, filters, setFilters, setActiveRoutes } = useActiveRoutes(
    routes,
    selectedPresetId,
  );
      ...
      try {
        ...
        if (savedFilters) {
          ...
          if (parsedFilters.selectedPresetId) {
            console.log(
              '🗺️ [MapScreen] Restoring saved collection selection:',
              parsedFilters.selectedPresetId,
            );
    ...
    const filteredCoordinates = activeRoutes
      .map((route) => {
        ...
        if (!isValidLat || !isValidLng || !isInSweden) {
          console.warn(
            '🚨 [MapScreen] Invalid/non-Swedish coordinates for route:',
            route.id,
            route.name,
            {
              lat,
  ...
  const handleLocationSelect = useCallback(
    async (result: SearchResult) => {
      ...
      try {
        ...
        console.log(
          '🗺️ [MapScreen] Filtered routes by location:',
        ...
        try {
          ...
          console.log(
            '🗑️ [MapScreen] Cleared saved filters from storage due to location selection',
          );
        } catch (error) {
          console.error(
            '❌ [MapScreen] Failed to clear saved filters due to location selection:',
            error,
          );
        }
  ...
  useEffect(() => {
    const handleRouteParams = async () => {
      ...
      if (selectedPresetId && fromHomeScreen) {
        ...
        try {
          ...
          console.error(
            '❌ [MapScreen] Failed to clear saved filters due to preset selection:',
            error,
          );
        }
    ...
    route?.params?.selectedPresetId,
    route?.params?.presetName,
    route?.params?.fromSearch,
    route?.params?.fromHomeScreen,
    route?.params?.ts,
        ...
        try {
          ...
          if (location && mapRef.current) {
            ...
            setTimeout(() => {
              if (mapRef.current) {
                console.log(
                  '🗺️ [MapScreen] Forcing marker re-render after locate me (permission granted)',
                );
                const currentRegion = { ...newRegion };
                mapRef.current.animateToRegion(currentRegion, 100);
              }
          ...
          console.log(
            '📍 [MapScreen] Location failed after permission granted, using Lund fallback',
          );
          const fallbackRegion = {
            latitude: 55.7047,
            longitude: 13.191,
  ...
  const handleApplyFilters = useCallback(
    (filters: FilterOptions) => {
      ...
      if (filters.selectedPresetId !== undefined) {
        ...
        console.log(
          '🗺️ [MapScreen] Updated selectedPresetId from filters:',
  ...
  return (
    <Screen edges={[]} padding={false} hideStatusBar scroll={false}>
      <AppHeader
        ...
        filterCounts={availableFilters.reduce(
          (acc, filter) => {
      ...
      <RouteDetailSheet
        ...
        onReopen={() => {
          console.log(
            '🎯 MapScreen: Reopening RouteDetailSheet - selectedRouteId:',
            selectedRouteId,
          );
          if (selectedRouteId) {
            setShowRouteDetailSheet(true);
