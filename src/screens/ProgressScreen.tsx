...
// Steps Accordion Component
interface ExerciseStepsAccordionProps {
  exercise: PathExercise;
  language: 'en' | 'sv';
}

const ExerciseStepsAccordion: React.FC<ExerciseStepsAccordionProps> = ({ exercise, language }) => {
  ...
  return (
    <YStack backgroundColor={colorScheme === 'dark' ? '#333' : '#F8F9FA'} borderRadius={12} overflow="hidden">
      {/* Header */}
      ...
      {isExpanded && (
        <YStack padding={16} paddingTop={0} gap={12}>
          {steps
            ...
            .map((step, index) => (
              <XStack key={index} alignItems="flex-start" gap={12}>
                {/* Step Number */}
                <View 
                  style={{
                    width: 28,
...
const StepMediaComponent: React.FC<StepMediaComponentProps> = ({ media, language }) => {
  const { effectiveTheme } = useThemePreference();
  const colorScheme = effectiveTheme === 'dark' ? 'dark' : 'light';

  const handleMediaPress = () => {
    if (media.type === 'youtube' || media.type === 'embed') {
