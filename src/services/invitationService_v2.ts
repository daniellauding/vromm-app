...
export async function inviteNewUser(
  ...
  try {
    ...
    const tempPassword =
      data.customPassword ||
      `Vromm${Math.random().toString(36).slice(2, 8)}${Math.floor(Math.random() * 100)}!`;
    ...
    try {
      ...
      const { data: sqlResult, error: sqlError } = await supabase.rpc(
        'create_invited_user_with_password',
        {
          p_email: email.toLowerCase(),
...
async function createSupervisorRelationship(
  ...
  try {
    ...
    const { error } = await supabase.from('student_supervisor_relationships').insert(relationship);
