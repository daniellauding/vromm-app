...
export function CollectionSharingModal({
  ...
  const handleUpdateUserRole = async (userId: string, newRole: 'viewer' | 'editor') => {
    ...
    try {
      ...
      showToast({
        ...
        message:
          t('routeCollections.roleUpdatedMessage') || 'User role has been updated successfully',
        type: 'success',
      });
  ...
  const handleCancelInvitation = async (userId: string, userName: string) => {
    ...
    try {
      ...
      showToast({
        ...
        message:
          t('collectionMembers.invitationCanceledMessage')?.replace('{userName}', userName) ||
          `Invitation for ${userName} has been canceled`,
        type: 'success',
      });
  ...
  const handleRemoveUser = async (userId: string, userName: string) => {
    ...
    try {
      ...
      showToast({
        ...
        message:
          t('routeCollections.userRemovedMessage')?.replace('{userName}', userName) ||
          `${userName} has been removed from the collection`,
        type: 'success',
      });
      ...
      showToast({
        title: t('common.error') || 'Error',
        message:
          t('routeCollections.failedToRemoveUser') || 'Failed to remove user from collection',
        type: 'error',
      });
    }
  };
  ...
  const handleUserLeaveCollection = async (
    userId: string,
    userName: string,
      ...
      for (const targetUser of selectedUsers) {
        ...
        try {
          ...
          if (existingInvitation) {
            console.log(
              '⚠️ [CollectionSharingModal] Pending invitation already exists:',
              targetUser.email,
            );
            duplicateCount++;
            continue;
          ...
          console.error(
            'Error processing collection sharing invitation for:',
            targetUser.email,
            error,
          );
          failCount++;
                  ...
                  >
                    <YStack gap="$4">
                      ...
                      {pendingCollectionInvitations.length > 0 && (
                        ...
                        >
                          <Text fontSize="$4" fontWeight="600" color="$color">
                            {t('routeCollections.collectionMembers') || 'Collection Members'} (
                            {pendingCollectionInvitations.length}):
                          </Text>
                          {pendingCollectionInvitations.map((invitation) => {
                            ...
                            const isCurrentUser =
                              invitation.metadata?.targetUserId === effectiveUserId;
                            ...
                            return (
                              ...
                              >
                                <XStack alignItems="center" justifyContent="space-between">
                                  <YStack flex={1}>
                                    <XStack alignItems="center" gap="$2">
                                      <Feather
                                        name={
                                          isCreator ? 'award' : isPending ? 'clock' : 'user-check'
                                        }
                                        size={16}
                                        color={
                                          isCreator ? '#FFD700' : isPending ? '#FFA500' : '#00E6C3'
                                        }
                                      />
                                      <Text fontSize="$4" fontWeight="600" color="$color">
                                    ...
                                    <XStack gap="$2" flexWrap="wrap" alignItems="center">
                                      <TouchableOpacity
                                        onPress={() =>
                                          handleUpdateUserRole(
                                            invitation.metadata?.targetUserId,
                                            'viewer',
                                          )
                                        }
                                        style={{
                                          ...
                                          borderRadius: 20,
                                          backgroundColor:
                                            currentRole === 'viewer' ? '#00E6C3' : 'transparent',
                                          borderWidth: 2,
                                          borderColor:
                                            currentRole === 'viewer' ? '#00E6C3' : '#666',
                                          minWidth: 80,
                                          alignItems: 'center',
                                        }}
                                        activeOpacity={0.7}
                                      ...
                                      <TouchableOpacity
                                        ...
                                        style={{
                                          ...
                                          borderRadius: 20,
                                          backgroundColor:
                                            currentRole === 'editor' ? '#00E6C3' : 'transparent',
                                          borderWidth: 2,
                                          borderColor:
                                            currentRole === 'editor' ? '#00E6C3' : '#666',
                                          minWidth: 80,
                                          alignItems: 'center',
                                        }}
                                        activeOpacity={0.7}
                                  ...
                                  <YStack gap="$2" marginTop="$2">
                                    ...
                                    >
                                      <XStack gap="$2" alignItems="center">
                                        ...
                                        <Text color="#EF4444">
                                          {t('routeCollections.leaveCollection') ||
                                            'Leave Collection'}
                                        </Text>
                                      </XStack>
                                    </Button>
                                ...
                                {isPending && canManage && (
                                  <YStack gap="$2" marginTop="$2">
                                    <Text fontSize="$3" color="$gray11" fontWeight="500">
                                      {t('collectionMembers.pendingInvitation') ||
                                        'Pending Invitation'}
                                      :
                                    </Text>
                      ...
                      <YStack gap="$2">
                        <Text fontSize="$3" color="$gray11">
                          {t('routeCollections.searchUsers') || 'Search for users:'}
                        </Text>
                        <Input
                          placeholder={
                            t('routeCollections.searchUsersPlaceholder') ||
                      ...
                      {selectedUsers.length > 0 && (
                        <YStack gap="$2">
                          <Text fontSize="$3" color="$gray11">
                            {t('routeCollections.selectedUsers') || 'Selected Users'} (
                      ...
                      <YStack gap="$2">
                        ...
                        <TextInput
                          ...
                          placeholder={
                            t('routeCollections.messagePlaceholder') || 'Add a personal message...'
                          }
                          multiline
                          style={{
                            backgroundColor: theme.background?.val || '#fff',
