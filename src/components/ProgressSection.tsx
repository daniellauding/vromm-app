...
export function ProgressSection({ activeUserId }: ProgressSectionProps) {
  ...
  > | null> => {
    try {
      ...
      if (saved) {
        ...
        console.log(
          '✅ [ProgressSection] Loaded saved filter preferences for user:',
          effectiveUserId,
          parsed,
        );
        return parsed;
  ...
  const handleExerciseSheetPress = React.useCallback(
    async (path: LearningPath) => {
      console.log(
        '🎯 [ProgressSection] Opening ExerciseListSheet for path:',
                  ...
                  <>
                    ...
                    <XStack gap={12} justifyContent="center">
                      ...
                      <TouchableOpacity
                        onPress={async () => {
                          console.log(
                            '💳 [ProgressSection] ==================== STRIPE PAYMENT FLOW ====================',
                          );
                          console.log(
                            '💳 [ProgressSection] Payment button pressed for path:',
                            paywallPath.title.en,
                          );
                          console.log(
                            '💳 [ProgressSection] Payment amount:',
                            paywallPath.price_usd || 1.0,
                          );
                          ...
                          try {
                            ...
                            const createPaymentIntent = async () => {
                              ...
                              if (error) {
                                ...
                                if (error instanceof FunctionsHttpError) {
                                  try {
                                    ...
                                    console.error(
                                      '💳 [ProgressSection] Edge function error details:',
                                      errorDetails,
                                    );
                                    realErrorMessage =
                                      errorDetails.error ||
                                    ...
                                    console.error(
                                      '💳 [ProgressSection] Failed to parse error context:',
                                      contextError,
                                    );
                                    try {
                                      const errorText = await error.context.text();
                                      console.error(
                                        '💳 [ProgressSection] Edge function error text:',
                                        errorText,
                                      );
                                      realErrorMessage = errorText || realErrorMessage;
                                    } catch (textError) {
                                      console.error(
                                        '💳 [ProgressSection] Failed to get error text:',
                                        textError,
                                      );
                                    }
                                  }
                              ...
                              if (data?.error) {
                                console.error(
                                  '💳 [ProgressSection] Edge function returned error:',
                                  data.error,
                                );

                                // FALLBACK: Create a properly formatted test payment intent
                                console.log(
                                  '💳 [ProgressSection] Creating fallback payment intent...',
                                );
                                return {
                                  paymentIntent:
                              ...
                              console.log(
                                '✅ [ProgressSection] Real payment intent created:',
                                data,
                              );
                              ...
                              if (
                                ...
                                console.error(
                                  '💳 [ProgressSection] Invalid response format - missing required fields:',
                                  {
                            ...
                            console.log(
                              '💳 [ProgressSection] Payment intent created:',
                              paymentData.paymentIntentClientSecret,
                            );

                            // Initialize PaymentSheet with proper structure
                            console.log(
                              '💳 [ProgressSection] Initializing PaymentSheet with data:',
                              {
                            ...
                            if (initError) {
                              console.error(
                                '💳 [ProgressSection] PaymentSheet init error:',
                                initError,
                              );
                              showToast({
                            ...
                            if (paymentError) {
                              console.log(
                                '💳 [ProgressSection] Payment was cancelled or failed:',
                                paymentError,
                              );
                            ...
                            if (!error) {
                              console.log(
                                '✅ [ProgressSection] Payment record created:',
                                paymentRecord.id,
                              );
                              showToast({
                              ...
                              console.error(
                                '❌ [ProgressSection] Error saving payment record:',
                                error,
                              );
                            }
                          } catch (error) {
